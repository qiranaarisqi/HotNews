...
<head>
    ...
    <title>User Dashboard - {{ config('app.name', 'HOT NEWS') }}</title>
    ...
    <div class="flex min-h-screen" x-data="{ sidebarOpen: true }">
        ...
        >
            ...
            <nav class="flex-grow p-4 space-y-2 mt-4 overflow-y-auto">
                <a href="{{ route('user.dashboard') }}" class="sidebar-link flex items-center p-3 rounded-xl transition group {{ request()->routeIs('user.dashboard') ? 'active' : 'hover:bg-white/5' }}">
                    <i data-lucide="layout-dashboard" class="w-5 h-5 flex-shrink-0"></i>
                    <span x-show="sidebarOpen" class="ml-4 font-medium transition-opacity">Overview</span>
                </a>

                <div x-show="sidebarOpen" class="px-4 py-2 mt-4 text-[10px] font-bold text-white/40 uppercase tracking-widest">Interactions</div>

                <a href="{{ route('user.comments.index') }}" class="sidebar-link flex items-center p-3 rounded-xl transition group {{ request()->routeIs('user.comments.*') ? 'active' : 'hover:bg-white/5' }}">
                    <i data-lucide="message-square" class="w-5 h-5 flex-shrink-0"></i>
                    <span x-show="sidebarOpen" class="ml-4 font-medium transition-opacity">My Comments</span>
                </a>
                
                <a href="{{ route('user.articles.index') }}" class="sidebar-link flex items-center p-3 rounded-xl transition group {{ request()->routeIs('user.articles.*') ? 'active' : 'hover:bg-white/5' }}">
                    <i data-lucide="file-text" class="w-5 h-5 flex-shrink-0"></i>
                    <span x-show="sidebarOpen" class="ml-4 font-medium transition-opacity">Articles I Commented</span>
                </a>

                <div x-show="sidebarOpen" class="px-4 py-2 mt-4 text-[10px] font-bold text-white/40 uppercase tracking-widest">Settings</div>

                <a href="{{ route('user.profile.edit') }}" class="sidebar-link flex items-center p-3 rounded-xl transition group {{ request()->routeIs('user.profile.*') ? 'active' : 'hover:bg-white/5' }}">
                    <i data-lucide="user" class="w-5 h-5 flex-shrink-0"></i>
                    <span x-show="sidebarOpen" class="ml-4 font-medium transition-opacity">Profile Settings</span>
                </a>
            </nav>
        ...
        >
            ...
            <header class="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-8 sticky top-0 z-40">
                <div class="flex items-center space-x-6">
                    ...
                    <div class="hidden md:flex flex-col">
                        <h2 class="text-sm font-bold text-gray-400 uppercase tracking-wider">User Dashboard</h2>
            ...
            <footer class="p-8 bg-white border-t border-gray-50 text-center text-sm text-gray-400 font-medium">
                &copy; {{ date('Y') }} HOT NEWS User Panel. Built for Excellence.
            </footer>
        </main>
    </div>
